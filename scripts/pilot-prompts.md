# Pilot: 6-Prompt Walkthrough

A curated set of prompts that exercise each sanitizer tier, Cache A, and the oversize skip (M1-11). The fail-closed audit posture is described last as a *planned* behavior (not yet wired in the pilot build) so you can speak to the v1 design without live-firing a feature that isn't there.

## Stage 0 — Presenter Setup

**Two-shell layout:**
- **Left shell:** your laptop, running Claude Code
- **Right shell/browser:** http://localhost:3000 (Langfuse OSS UI), login with `demo@corp.lan` / `demo-password-12345` (pre-seeded via `LANGFUSE_INIT_*` in `docker-compose.pilot.yml`)

**Second shell environment:**
Run this in the second shell (the one where you'll interact with Claude Code) to route traffic through the pilot gateway:
...
Alternatively, run `scripts/pilot.sh presenter-env` and copy-paste the output.
...
```bash
docker compose -f docker-compose.pilot.yml exec litellm \
...
Langfuse at all — only requests that flow through the gateway on `:8080` emit an
audit trace.
...
The remaining v1 commitment that *this pilot build does not yet exercise live* is the fail-closed posture on the audit pipeline. Walk colleagues through it as a design promise, not a live click-through, because the runtime guard hasn't been wired into `CorpLlmGuardrail` yet.
...
When you're done with the pilot:

```bash
# Preserve state (volumes kept, containers stopped)
scripts/pilot.sh down

# OR nuke everything (clean slate for next pilot)
