...
Полный поток данных на каждый запрос:

![Поток данных запроса/ответа corp-llm-gateway](docs/data-flow.png)

Исходник: [`docs/data-flow.puml`](docs/data-flow.puml) (перерендерить командой `plantuml docs/data-flow.puml`).
...
Живой файл правил для демо — `docker/demo-litellm/rules/demo-team.md`. Полная спецификация и советы по написанию: [`docs/replace-md-authoring.md`](docs/replace-md-authoring.md).
...
docs/                   план + audit-schema + ops/* + rbac-matrix + data-flow + документы по интеграции
...
Три паттерна интеграции в зависимости от вашего харнесса — полные рецепты в [`docs/harness-integration.md`](docs/harness-integration.md):
...
Чтобы ротировать вручную до истечения срока, повторно запустите `install.sh`. Полный жизненный цикл потока токена, модель свежести и карта режимов отказа: [`docs/x-corp-auth.md`](docs/x-corp-auth.md).
...
Откат: `helm rollback gw <revision>` (Helm хранит последние 10). Полный процесс релиза + откат — в [`docs/ops/runbook.md`](docs/ops/runbook.md).
...
Источник истины: [`docs/ops/runbook.md`](docs/ops/runbook.md) (плейбук инцидентов, матрица fail-policy, типовые операции вроде `gateway-admin team create`, `gateway-admin token revoke`, однострочники kubectl).

Расчёт мощностей по фазам (Phase 0 alpha → Phase 3 GA при 1000 разработчиков / 50 RPS суммарно): [`docs/ops/capacity.md`](docs/ops/capacity.md).
...
в [`docs/demo.md`](docs/demo.md). Демо-стек лежит в `docker-compose.demo.yml`
...
| [`docs/adr/ADR-003-ner-orchestration.md`](docs/adr/ADR-003-ner-orchestration.md) | ADR: hand-roll dual-NER (Natasha RU + spaCy EN) вместо Presidio/DeepPavlov |
| [`docs/data-flow.puml`](docs/data-flow.puml) + [`docs/data-flow.png`](docs/data-flow.png) | сквозная диаграмма последовательности (исходник PlantUML + отрендеренный PNG) |
| [`docs/harness-integration.md`](docs/harness-integration.md) | рецепты настройки по каждому харнессу (Claude Code, Codex, Cursor, …) |
| [`docs/x-corp-auth.md`](docs/x-corp-auth.md) | жизненный цикл корп-токена, свежесть по паттернам, режимы отказа |
| [`docs/conversation-id.md`](docs/conversation-id.md) | текущее поведение `conversation_id` + как подключить настоящий session ID |
| [`docs/audit-schema.md`](docs/audit-schema.md) | схема события аудита + классификация полей ALWAYS / CONDITIONAL / NEVER |
| [`docs/security.md`](docs/security.md) | покрытие sanitization, гарантии конвейера аудита, обработка SIEM / Langfuse, известные пробелы в конфигурации |
| [`docs/replace-md-authoring.md`](docs/replace-md-authoring.md) | как писать файлы правил `replace.md` для команды |
| [`docs/rbac-matrix.md`](docs/rbac-matrix.md) | кто что может (разработчики / тимлиды / операторы / безопасность) |
| [`docs/remaining-steps.md`](docs/remaining-steps.md) | текущий чек-лист оставшейся работы по v1 |
| [`docs/ops/runbook.md`](docs/ops/runbook.md) | релиз, откат, плейбук инцидентов, типовые операции |
| [`docs/ops/capacity.md`](docs/ops/capacity.md) | расчёт мощностей по фазам раскатки (alpha → GA) |
