...
Это дополняет [`harness-integration.md`](harness-integration.ru.md), который
...
### Когда включён мост подписочной аутентификации

`CORP_LLM_FORWARD_ANTHROPIC_AUTH` (и его близнец для ChatGPT Codex) ничего
не меняет для `X-Corp-Auth`: он по-прежнему поглощается в `pre_call`,
по-прежнему срезается из всех наборов пробрасываемых заголовков и,
дополнительно, явно исключён из allowlist-а заголовков этого моста.

Меняется судьба *другого* credential-а. Мост **читает** входящий bearer из
`Authorization` и копирует его значение в `data["api_key"]` — именно это
заставляет litellm собрать исходящий заголовок `Authorization: Bearer` из
подписочного OAuth-токена. Сам входящий заголовок остаётся ровно таким,
каким пришёл, поэтому инвариант #3 не меняется. Два credential-а едут в
разных заголовках и никогда не конфликтуют — ради этого разделения корп-токен
и получил собственный заголовок.

Настройка и полный список «что пробрасывается, а что нет»:
[`ops/install.md`](ops/install.md) и [`security.md`](security.ru.md) §13.
