# Демо (разбор на ноутбуке)

## Аудитория и длительность

Это ~15-минутный разбор в терминале для коллег — из ИБ, эксплуатации, смежных команд — которые хотят увидеть шлюз в действии, не читая код. Демо запускается на вашем ноутбуке: Claude Code слева, настоящий UI аудита Langfuse OSS в браузере справа. Трафик идёт через настоящую корп-LLM (через демо-прокси LiteLLM), и каждый запрос проходит аудит и виден в Langfuse.

## Предварительные требования

- Docker 24+ и Docker Compose v2
- `jq` и `curl` (использует `scripts/demo.sh` для опроса healthcheck и настройки Langfuse; `brew install jq` на macOS, `apt install jq` на Debian/Ubuntu)
- ~3 GB свободной RAM
...
2. Скопируйте и настройте демо-файл окружения:
   ```bash
   ...
   сертификат, подписанный внутренним корпоративным CA, поэтому демо доверяет этой
   ...
   прописаны в `docker-compose.demo.yml` — менять `.env.demo` не нужно. (Старого
...
3. Холодный старт стенда (~3–5 минут при первом запуске):
   ...
   scripts/demo.sh up
   ```
   Это скачивает образы, создаёт тома, инициализирует Langfuse демо-учётками и печатает URL-ы.

4. Запомните напечатанные URL: http://localhost:4000 (прокси LiteLLM), http://localhost:3000 (Langfuse).
...
**Правый шелл/браузер:** откройте вкладку браузера на http://localhost:3000. Войдите с учётными данными, напечатанными командой `up` (или посмотрите в `.env.demo` значения `LANGFUSE_PUBLIC_KEY` / `LANGFUSE_SECRET_KEY`).

**Чтобы направить Claude Code на демо-прокси**, выполните это в левом шелле:
```bash
...
Каждый промпт задействует свою часть шлюза. Полный текст промптов и детали настройки — в [`scripts/demo-prompts.md`](../scripts/demo-prompts.md).
...
См. [`scripts/demo-prompts.md`](../scripts/demo-prompts.md) для точного текста, ожидаемых наблюдений в Langfuse и того, как интерпретировать каждый трейс.
...
- **Корп-LLM недоступна** — проверьте связь по VPN. Команда `scripts/demo.sh up` предупреждает, но не падает; стенд будет здоров, даже если корп-LLM лежит. Реальная отправка промптов упадёт с ошибкой шлюза. Проверка: `curl -v https://<your-corp-llm-endpoint>/health`.

- **Трейсы в Langfuse не появляются** — шаг seed-langfuse мог упасть (редко, обычно сетевые таймауты). Запустите `scripts/demo.sh seed-langfuse` вручную для идемпотентного повтора. Затем перезапустите сервис vector: `docker compose -f docker-compose.demo.yml restart vector`.

- **HTTP 503 с телом «audit pipeline unhealthy»** — это **намеренно** в промпте #7 (демонстрирует fail-closed-поведение при остановке Vector). Если появляется неожиданно, проверьте статус Vector: `docker compose -f docker-compose.demo.yml ps vector` и логи: `docker compose -f docker-compose.demo.yml logs vector`.

- **Контейнер LiteLLM не стартует** — обычно сетевая проблема во время `pip install -e /pkg`. Проверьте логи: `docker compose -f docker-compose.demo.yml logs litellm`. Повторите `scripts/demo.sh up`.
...
- ❌ S3-sink или SIEM-sink аудита (в демо единственный sink — Langfuse)
- ❌ Изоляцию нескольких команд (одна команда: `demo-team`)
- ❌ Правила `replace.md` по командам (один файл правил по умолчанию)
- ❌ CI-джобу `e2e:langfuse` или существующий `docker-compose.yml` (демо-стенд параллельный, независимый)

Это продовые заботы; демо фокусируется на ключевом потоке вымарывание→аудит→восстановление.
