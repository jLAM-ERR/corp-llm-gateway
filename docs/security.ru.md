...
живут только в docker json-file логе контейнера `litellm` плюс то, что уже
попало в дисковый буфер Vector. Glob файлового источника Vector — `*/*-json.log`
— и он не следует за ротированными файлами `-json.log.1`, поэтому **здесь
долговечность аудита ограничивает ротация docker-логов, а не дисковый буфер**:
ротация, обогнавшая Vector, теряет эти записи безвозвратно. Отдельно: HTTP-sink
Vector повторяет только 408/429/5xx, поэтому неверный или ротированный
project-ключ Langfuse (401) приводит к отбрасыванию записи, а не к буферизации.

Поэтому операторы этого стека обязаны рассчитать retention docker-логов
(`max-size` × `max-file` для контейнера `litellm`) на самый долгий допустимый
простой Langfuse и настроить алерты на рост буфера и на ошибки
`Events dropped` в Vector. Конкретные команды: `compose/README.md`,
