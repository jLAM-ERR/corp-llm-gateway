...
5. **Фабрика читает только env-переменные**, никаких неявных дефолтов на основании окружения. Неизвестное значение — `ValueError` с перечислением допустимых.
...
- **Юнит-тестируемость без инфраструктуры.** 265 тестов исполняются за ≈16 секунд без Redis, Postgres, corp LLM или k8s.
