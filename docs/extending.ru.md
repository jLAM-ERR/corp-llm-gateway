...
`corp_ner` — единственный встроенный детектор, ходящий **по сети**: при его
выборе обязателен `CORP_NER_ENDPOINT` (без него шлюз откажется стартовать), и
он исключён из сегментов `CODE`. Локальные детекторы из `CODE` НЕ исключаются:
именно локальный NER ловит PERSON/ORG внутри JSON в блоках кода, значений SQL
и примеров конфигов.
...
6. **Реализуйте `policy_signature()`, если класс не определяет ваше покрытие.** Ключ Cache A
   включает отпечаток эффективной политики, а детектор опознаётся в нём только по import-пути +
   qualname. Если два экземпляра вашего класса могут редактировать по-разному — опциональные
   модели, которых может не быть; аргументы конструктора, сужающие покрытие; удалённый endpoint —
   добавьте опциональный хук `PolicySignatureDetector` (`detectors/base.py`) и верните эту разницу:
   ...
   Правила: простые отсортированные строки, никаких встроенного `hash()` / `id()` / `repr()`
   объекта / порядка обхода set (они различаются от процесса к процессу, а pod'ы делят один
   Redis); никакого пользовательского контента (M1-14); и значение не должно меняться после
   конструирования — разрешайте всё ленивое внутри хука, чтобы оно «защёлкнулось», как это делают
   `ner_ru` / `ner_en`. Исключение или не-строка отключают Cache A целиком для этого
   оркестратора (fail closed). Пропустите хук — унаследуете идентичность класса, что корректно
   только тогда, когда все экземпляры класса редактируют одинаково.
